...
public class MrzDate implements Serializable, Comparable<MrzDate> {
    ...
    private static Logger log = Logger.getLogger(MrzDate.class.getSimpleName());
    ...
    private boolean check() {
        if (year < 0 || year > 99) {
            log.info("Parameter year: invalid value " + year + ": must be 0..99");
            return false;
        }
        if (month < 1 || month > 12) {
            log.info("Parameter month: invalid value " + month + ": must be 1..12");
            return false;
        }
        if (day < 1 || day > 31) {
            log. info("Parameter day: invalid value " + day + ": must be 1..31");
            return false;
        }
